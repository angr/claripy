...
def normalize_types(f):
    ...
    def normalizer(self, o):
        ...
        def _lossless_reverse(a):
            return a.is_top or a.is_integer
...
class StridedInterval(BackendObject):
    ...
    *Signedness-Agnostic Program Analysis: Precise Integer Bounds for Low-Level
    ...
    def __init__(self, name=None, bits=0, stride=None, lower_bound=None, upper_bound=None, bottom=False):
        self._name = name
    ...
    def _rshift_logical(self, shift_amount):
        ...
        if len(ssplit) == 1:
            ...
            return StridedInterval(bits=self.bits, lower_bound=l, upper_bound=u, stride=stride)
    ...
    def add(self, b):
        ...
        return StridedInterval(bits=new_bits, stride=stride, lower_bound=lb, upper_bound=ub).normalize()
    ...
    def cast_low(self, tok):
        ...
        if 0 <= (self.upper_bound - self.lower_bound) <= mask:
            ...
            return StridedInterval(bits=tok, stride=self.stride, lower_bound=l, upper_bound=u)
        ...
        if (self.upper_bound & mask == self.lower_bound & mask) and ((self.upper_bound - self.lower_bound) & mask == 0):
            ...
            return StridedInterval(bits=tok, stride=0, lower_bound=bound, upper_bound=bound)
    ...
    def pseudo_join(s, b, smart_join=True):
        ...
        if not smart_join:
            if s.is_integer:
                ...
                new_stride = StridedInterval.gcd(s.stride, b.stride)
                new_stride = StridedInterval.gcd(
        ...
        # from b to s
        new_stride1 = StridedInterval.gcd(
            new_stride, StridedInterval._wrapped_cardinality(b.lower_bound, s.lower_bound, w) - 1
        )
        # from s to b
        new_stride2 = StridedInterval.gcd(
            new_stride, StridedInterval._wrapped_cardinality(s.lower_bound, b.lower_bound, w) - 1
        )
        si1 = StridedInterval(
            bits=w,
    ...
    def _involuted_reverse(self):
        ...
        si = StridedInterval(bits=o.bits, lower_bound=si_lb, upper_bound=si_ub, stride=o._stride)
...
def CreateStridedInterval(  # pylint: disable=too-many-positional-arguments
    ...
    to_conv=None,
    discrete_set=False,
    discrete_set_max_cardinality=None,
):
    """
    :param name:
    :param bits:
