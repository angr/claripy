...
def raw_hybrid_solver(reuse_z3_solver):
    ...
    old_solve_count = claripy.backends.backend_z3.solve_count
    assert sorted(s.eval(x, 20)) == [0, 2, 4, 6, 8, 10]
    assert claripy.backends.backend_z3.solve_count == old_solve_count
...
def raw_solver(solver_type, reuse_z3_solver):
    ...
    if isinstance(s, claripy.frontend_mixins.ModelCacheMixin):
        count = claripy.backends.backend_z3.solve_count

        s = solver_type()
        x = claripy.BVS("x", 32)
        s.add(x == 10)
        assert s.satisfiable()
        assert claripy.backends.backend_z3.solve_count == count
...
def raw_composite_solver(reuse_z3_solver):
    ...
    if isinstance(s._template_frontend, claripy.frontend_mixins.ModelCacheMixin):
        ...
        count = claripy.backends.backend_z3.solve_count
        assert s.satisfiable()
        assert claripy.backends.backend_z3.solve_count == count + 3
