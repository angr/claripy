...
class TestVSAJoin(unittest.TestCase):  # pylint: disable=no-member,function-redefined
    ...
    def setUp(self):
        ...
        self.SI = claripy.SI
        self.a = self.SI(bits=8, to_conv=2)
        self.b = self.SI(bits=8, to_conv=10)
        self.c = self.SI(bits=8, to_conv=120)
        self.d = self.SI(bits=8, to_conv=130)
    ...
    def test_union_5_elements(self):
        ...
        assert claripy.backends.vsa.identical(tmp2, self.SI(bits=8, stride=2, lower_bound=2, upper_bound=120))
        tmp3 = tmp2.union(self.d).union(self.e)
        assert claripy.backends.vsa.identical(tmp3, self.SI(bits=8, stride=2, lower_bound=2, upper_bound=132))
    ...
    def test_union_8_elements(self):
        ...
        c = self.SI(bits=8, to_conv=120)
        d = self.SI(bits=8, to_conv=130)
        e = self.SI(bits=8, to_conv=132)
        f = self.SI(bits=8, to_conv=135)
        g = self.SI(bits=8, to_conv=220)
        h = self.SI(bits=8, to_conv=50)

        tmp = a.union(b).union(c).union(d).union(e).union(f).union(g).union(h)
        assert claripy.backends.vsa.identical(tmp, self.SI(bits=8, stride=1, lower_bound=220, upper_bound=135))
...
class TestVSAOperations(unittest.TestCase):  # pylint: disable=no-member function-redefined
    ...
    def setUp(self):
        ...
        self.si_c = self.SI(bits=32, stride=3, lower_bound=-100, upper_bound=200)
        self.si_d = self.SI(bits=32, stride=2, lower_bound=50, upper_bound=60)
        self.si_e = self.SI(bits=16, stride=1, lower_bound=0x2000, upper_bound=0x3000)
        self.si_f = self.SI(bits=16, stride=1, lower_bound=0, upper_bound=255)
        self.si_g = self.SI(bits=16, stride=1, lower_bound=0, upper_bound=0xFF)
        self.si_h = self.SI(bits=32, stride=0, lower_bound=0x80000000, upper_bound=0x80000000)
    ...
    def test_normalization(self):
        ...
        si1_normalized = self.SI(bits=32, stride=1, lower_bound=10, upper_bound=10)
        assert vsa_model(si1_normalized).stride == 0

    def test_integers(self):
        # Test for Integers
        assert self.is_equal(self.si1, self.SI(bits=32, to_conv=10))
        assert self.is_equal(self.si2, self.SI(bits=32, to_conv=10))
        assert self.is_equal(self.si1, self.si2)

    def test_strided_intervals(self):
        # Strided interval assertions could go here if needed in future
        pass

    def test_add_operations(self):
        # __add__ operations
        si_add_1 = self.si1 + self.si2
        assert self.is_equal(si_add_1, self.SI(bits=32, stride=0, lower_bound=20, upper_bound=20))
        si_add_2 = self.si1 + self.si_a
        assert self.is_equal(si_add_2, self.SI(bits=32, stride=2, lower_bound=20, upper_bound=30))
        si_add_3 = self.si_a + self.si_b
        assert self.is_equal(si_add_3, self.SI(bits=32, stride=2, lower_bound=-90, upper_bound=220))
        si_add_4 = self.si_b + self.si_c
        assert self.is_equal(si_add_4, self.SI(bits=32, stride=1, lower_bound=-200, upper_bound=400))
    ...
    def test_add_with_overflow(self):
        ...
        assert self.is_equal(
            si_add_5,
            self.SI(bits=32, stride=0, lower_bound=0x7FFFFFFF, upper_bound=0x7FFFFFFF),
        )

    def test_sub_operations(self):
        # __sub__ operations
        si_minus_1 = self.si1 - self.si2
        assert self.is_equal(si_minus_1, self.SI(bits=32, stride=0, lower_bound=0, upper_bound=0))
        si_minus_2 = self.si_a - self.si_b
        assert self.is_equal(si_minus_2, self.SI(bits=32, stride=2, lower_bound=-190, upper_bound=120))
        si_minus_3 = self.si_b - self.si_c
        assert self.is_equal(si_minus_3, self.SI(bits=32, stride=1, lower_bound=-300, upper_bound=300))

    def test_neg_invert(self):
        # __neg__ / __invert__ / bitwise not
        si_neg_1 = ~self.si1
        assert self.is_equal(si_neg_1, self.SI(bits=32, to_conv=-11))
        si_neg_2 = ~self.si_b
        assert self.is_equal(si_neg_2, self.SI(bits=32, stride=2, lower_bound=-201, upper_bound=99))

    def test_or_operations(self):
        # __or__ operations
        si_or_1 = self.si1 | self.si3
        assert self.is_equal(si_or_1, self.SI(bits=32, to_conv=30))
        si_or_2 = self.si1 | self.si2
        assert self.is_equal(si_or_2, self.SI(bits=32, to_conv=10))
        si_or_3 = self.si1 | self.si_a  # An integer | a strided interval
        assert self.is_equal(si_or_3, self.SI(bits=32, stride=2, lower_bound=10, upper_bound=30))
        si_or_3 = self.si_a | self.si1  # Exchange the operands
        assert self.is_equal(si_or_3, self.SI(bits=32, stride=2, lower_bound=10, upper_bound=30))
        si_or_4 = self.si_a | self.si_d  # A strided interval | another strided interval
        assert self.is_equal(si_or_4, self.SI(bits=32, stride=2, lower_bound=50, upper_bound=62))
        si_or_4 = self.si_d | self.si_a  # Exchange the operands
        assert self.is_equal(si_or_4, self.SI(bits=32, stride=2, lower_bound=50, upper_bound=62))
        si_or_5 = self.si_e | self.si_f
        assert self.is_equal(si_or_5, self.SI(bits=16, stride=1, lower_bound=0x2000, upper_bound=0x30FF))
        si_or_6 = self.si_e | self.si_g
        assert self.is_equal(si_or_6, self.SI(bits=16, stride=1, lower_bound=0x2000, upper_bound=0x30FF))

    def test_shifting(self):
        # Shifting operations
        si_shl_1 = self.si1 << 3
        assert si_shl_1.size() == 32
        assert self.is_equal(si_shl_1, self.SI(bits=32, stride=0, lower_bound=80, upper_bound=80))

    def test_multiplication(self):
        # Multiplication operations
        si_mul_1 = self.si1 * 3
        assert si_mul_1.size() == 32
        assert self.is_equal(si_mul_1, self.SI(bits=32, stride=0, lower_bound=30, upper_bound=30))
        si_mul_2 = self.si_a * 3
        assert si_mul_2.size() == 32
        assert self.is_equal(si_mul_2, self.SI(bits=32, stride=6, lower_bound=30, upper_bound=60))
        si_mul_3 = self.si_a * self.si_b
        assert si_mul_3.size() == 32
        assert self.is_equal(si_mul_3, self.SI(bits=32, stride=2, lower_bound=-2000, upper_bound=4000))

    def test_division(self):
        # Division operations
        si_div_1 = self.si1 // 3
        assert si_div_1.size() == 32
        assert self.is_equal(si_div_1, self.SI(bits=32, stride=0, lower_bound=3, upper_bound=3))
        si_div_2 = self.si_a // 3
        assert si_div_2.size() == 32
        assert self.is_equal(si_div_2, self.SI(bits=32, stride=1, lower_bound=3, upper_bound=6))

    def test_modulo(self):
        # Modulo operations
        si_mo_1 = self.si1 % 3
        assert si_mo_1.size() == 32
        assert self.is_equal(si_mo_1, self.SI(bits=32, stride=0, lower_bound=1, upper_bound=1))
        si_mo_2 = self.si_a % 3
        assert si_mo_2.size() == 32
        assert self.is_equal(si_mo_2, self.SI(bits=32, stride=1, lower_bound=0, upper_bound=2))

    def test_sign_bit_negative_integer(self):
        # Extracting the sign bit from a negative integer
        si = self.SI(bits=64, stride=0, lower_bound=-1, upper_bound=-1)
        sb = si[63:63]
        assert self.is_equal(sb, self.SI(bits=1, to_conv=1))

    def test_sign_bit_non_positive_integers(self):
        # Extracting the sign bit from non-positive integers
        si = self.SI(bits=64, stride=1, lower_bound=-1, upper_bound=0)
        sb = si[63:63]
        assert self.is_equal(sb, self.SI(bits=1, stride=1, lower_bound=0, upper_bound=1))
    ...
    def test_extracting_integer(self):
        ...
        assert self.is_equal(
            part2,
            self.SI(bits=32, stride=0, lower_bound=0xFFFF0000, upper_bound=0xFFFF0000),
        )
    ...
    def test_extracting_si(self):
        ...
        si = self.SI(bits=64, stride=0x9, lower_bound=0x1, upper_bound=0xA)
        part1 = si[63:32]
        part2 = si[31:0]
        assert self.is_equal(part1, self.SI(bits=32, stride=0, lower_bound=0x0, upper_bound=0x0))
        assert self.is_equal(part2, self.SI(bits=32, stride=9, lower_bound=1, upper_bound=10))
    ...
    def test_concatenating_two_si_different_sizes(self):
        ...
        si_1 = self.SI(bits=64, stride=1, lower_bound=0, upper_bound=0xFFFFFFFFFFFFFFFF)
        si_2 = self.SI(bits=32, stride=1, lower_bound=0, upper_bound=0xFFFFFFFF)
        si_concat = si_1.concat(si_2)
        assert self.is_equal(
            si_concat,
            self.SI(bits=96, stride=1, lower_bound=0, upper_bound=0xFFFFFFFFFFFFFFFFFFFFFFFF),
        )
    ...
    def test_zero_extend(self):
        ...
        assert self.is_equal(si_zeroextended, self.SI(bits=64, stride=9, lower_bound=1, upper_bound=10))
    ...
    def test_sign_extend(self):
        ...
        assert self.is_equal(si_signextended, self.SI(bits=64, stride=9, lower_bound=1, upper_bound=10))
    ...
    def test_extract_from_zero_extended(self):
        ...
        assert self.is_equal(si_extracted, self.SI(bits=32, stride=9, lower_bound=1, upper_bound=10))

    def test_union_operations(self):
        # Union operations
        si_union_1 = self.si1.union(self.si2)
        assert self.is_equal(si_union_1, self.SI(bits=32, stride=0, lower_bound=10, upper_bound=10))

        si_union_2 = self.si1.union(self.si3)
        assert self.is_equal(si_union_2, self.SI(bits=32, stride=18, lower_bound=10, upper_bound=28))

        si_union_3 = self.si1.union(self.si_a)
        assert self.is_equal(si_union_3, self.SI(bits=32, stride=2, lower_bound=10, upper_bound=20))

        si_union_4 = self.si_a.union(self.si_b)
        assert self.is_equal(si_union_4, self.SI(bits=32, stride=2, lower_bound=-100, upper_bound=200))

        si_union_5 = self.si_b.union(self.si_c)
        assert self.is_equal(si_union_5, self.SI(bits=32, stride=1, lower_bound=-100, upper_bound=200))
    ...
    def test_intersection_operations(self):
        ...
        assert self.is_equal(si_intersection_2, self.SI(bits=32, stride=0, lower_bound=10, upper_bound=10))

        si_intersection_3 = self.si1.intersection(self.si_a)
        assert self.is_equal(si_intersection_3, self.SI(bits=32, stride=0, lower_bound=10, upper_bound=10))

        si_intersection_4 = self.si_a.intersection(self.si_b)
        assert self.is_equal(si_intersection_4, self.SI(bits=32, stride=2, lower_bound=10, upper_bound=20))

        si_intersection_5 = self.si_b.intersection(self.si_c)
        assert self.is_equal(
            si_intersection_5,
            self.SI(bits=32, stride=6, lower_bound=-100, upper_bound=200),
        )

    def test_more_intersections(self):
        # More intersections
        t0 = self.SI(bits=32, stride=1, lower_bound=0, upper_bound=0x27)
        t1 = self.SI(bits=32, stride=0x7FFFFFFF, lower_bound=0x80000002, upper_bound=1)

        si_is_6 = t0.intersection(t1)
        assert self.is_equal(si_is_6, self.SI(bits=32, stride=0, lower_bound=1, upper_bound=1))

        t2 = self.SI(bits=32, stride=5, lower_bound=20, upper_bound=30)
        t3 = self.SI(bits=32, stride=1, lower_bound=27, upper_bound=0xFFFFFFFF)

        si_is_7 = t2.intersection(t3)
        assert self.is_equal(si_is_7, self.SI(bits=32, stride=0, lower_bound=30, upper_bound=30))

        t4 = self.SI(bits=32, stride=5, lower_bound=-400, upper_bound=400)
        t5 = self.SI(bits=32, stride=1, lower_bound=395, upper_bound=-395)

        si_is_8 = t4.intersection(t5)
        assert self.is_equal(si_is_8, self.SI(bits=32, stride=5, lower_bound=-400, upper_bound=400))

    def test_sign_extension(self):
        # Sign-extension
        si = self.SI(bits=1, stride=0, lower_bound=1, upper_bound=1)
        si_signextended = si.sign_extend(31)
        assert self.is_equal(
            si_signextended,
            self.SI(bits=32, stride=0, lower_bound=0xFFFFFFFF, upper_bound=0xFFFFFFFF),
        )

    def test_comparison_si_bvv(self):
        # Comparison between claripy.SI and BVV
        si = self.SI(bits=32, stride=1, lower_bound=-0x7F, upper_bound=0x7F)
        claripy.backends.vsa.convert(si).uninitialized = True
        bvv = self.BVV(0x30, 32)
        comp = si < bvv
        assert claripy.backends.vsa.convert(comp).identical(claripy.vsa.MaybeResult())

    def test_better_extraction(self):
        # Better extraction
        si = self.SI(bits=32, stride=0x1000000, lower_bound=0xCFFFFFF, upper_bound=0xDFFFFFF)
        si_byte0 = si[7:0]
        si_byte1 = si[15:8]
        si_byte2 = si[23:16]
        si_byte3 = si[31:24]
        assert self.is_equal(si_byte0, self.SI(bits=8, stride=0, lower_bound=0xFF, upper_bound=0xFF))
        assert self.is_equal(si_byte1, self.SI(bits=8, stride=0, lower_bound=0xFF, upper_bound=0xFF))
        assert self.is_equal(si_byte2, self.SI(bits=8, stride=0, lower_bound=0xFF, upper_bound=0xFF))
        assert self.is_equal(si_byte3, self.SI(bits=8, stride=1, lower_bound=0xC, upper_bound=0xD))

    def test_bitwise_and_optimization(self):
        # Optimization on bitwise-and
        si_1 = self.SI(bits=32, stride=1, lower_bound=0x0, upper_bound=0xFFFFFFFF)
        si_2 = self.SI(bits=32, stride=0, lower_bound=0x80000000, upper_bound=0x80000000)
        si = si_1 & si_2
        assert self.is_equal(
            si,
            self.SI(bits=32, stride=0x80000000, lower_bound=0, upper_bound=0x80000000),
        )

        si_1 = self.SI(bits=32, stride=1, lower_bound=0x0, upper_bound=0x7FFFFFFF)
        si_2 = self.SI(bits=32, stride=0, lower_bound=0x80000000, upper_bound=0x80000000)
        si = si_1 & si_2
        assert self.is_equal(si, self.SI(bits=32, stride=0, lower_bound=0, upper_bound=0))

    def test_concatenation_with_zeros(self):
        # Concatenation: concat with zeros only increases the stride
        si_1 = self.SI(bits=8, stride=0xFF, lower_bound=0x0, upper_bound=0xFF)
        si_2 = self.SI(bits=8, stride=0, lower_bound=0, upper_bound=0)
        si = si_1.concat(si_2)
        assert self.is_equal(si, self.SI(bits=16, stride=0xFF00, lower_bound=0, upper_bound=0xFF00))

    def test_extraction_from_reversed_value(self):
        # Extract from a reversed value
        si_1 = self.SI(bits=64, stride=0xFF, lower_bound=0x0, upper_bound=0xFF)
    ...
    def test_value_set_operations(self):
        ...
        vsa_model(vs_1)._merge_si("global", 0, vsa_model(self.SI(bits=32, stride=0, lower_bound=10, upper_bound=10)))
        vsa_model(vs_1)._merge_si("global", 0, vsa_model(self.SI(bits=32, stride=0, lower_bound=28, upper_bound=28)))
        assert (
            vsa_model(vs_1)
            .get_si("global")
            .identical(vsa_model(self.SI(bits=32, stride=18, lower_bound=10, upper_bound=28)))
        )
        ...
        vsa_model(vs_2)._merge_si("global", 0, vsa_model(self.SI(bits=32, stride=0, lower_bound=10, upper_bound=10)))
    ...
    def test_if_proxy(self):
        # IfProxy
        si = self.SI(bits=32, stride=1, lower_bound=10, upper_bound=0xFFFFFFFF)
    ...
    def test_max_min_on_ifproxy(self):
        ...
        si = self.SI(bits=32, stride=1, lower_bound=0, upper_bound=0xFFFFFFFF)
        if_0 = claripy.If(si == 0, si, si - 1)
    ...
    def test_if_proxy_identical(self):
        ...
        si = self.SI(bits=32, stride=0, lower_bound=1, upper_bound=1)
        if_0 = claripy.If(si == 0, si, si - 1)
        if_0_copy = claripy.If(si == 0, si, si - 1)
        assert self.is_equal(if_0, if_0_copy)
        if_1 = claripy.If(si == 1, si, si - 1)
    ...
    def test_if_proxy_and_operations(self):
        ...
        vs_2 = self.VS(region="global", bits=32, val=0xFA7B00B)
        si = self.SI(bits=32, stride=1, lower_bound=0, upper_bound=1)
        if_1 = vs_2 & claripy.If(
            si == 0,
            self.SI(bits=32, stride=0, lower_bound=0, upper_bound=0),
            self.SI(bits=32, stride=0, lower_bound=0xFFFFFFFF, upper_bound=0xFFFFFFFF),
        )
        assert claripy.backends.vsa.is_true(
            vsa_model(if_1.ite_excavated.args[1]) == vsa_model(self.VS(region="global", bits=32, val=0))
        )
    ...
    def test_if_proxy_or_operations(self):
        ...
        vs_3 = self.VS(region="global", bits=32, val=0xDEADCA7)
        si = self.SI(bits=32, stride=1, lower_bound=0, upper_bound=1)
        ...
        assert claripy.backends.vsa.is_true(
            vsa_model(if_2.ite_excavated.args[1]) == vsa_model(self.VS(region="global", bits=32, val=0))
        )
...
class TestVSAConstraintToSI(unittest.TestCase):  # pylint: disable=no-member,function-redefined
    ...
    def test_if_si_equals_1(self):
        # If(SI == 0, 1, 0) == 1
        s1 = self.SI(bits=32, stride=1, lower_bound=0, upper_bound=2)
        ast_true = claripy.If(s1 == self.BVV(0, 32), self.BVV(1, 1), self.BVV(0, 1)) == self.BVV(1, 1)
        ast_false = claripy.If(s1 == self.BVV(0, 32), self.BVV(1, 1), self.BVV(0, 1)) != self.BVV(1, 1)
        ...
        assert claripy.backends.vsa.is_true(
            trueside_replacement[0][1] == self.SI(bits=32, stride=0, lower_bound=0, upper_bound=0)
        )
        ...
        assert claripy.backends.vsa.identical(
            falseside_replacement[0][1], self.SI(bits=32, stride=1, lower_bound=1, upper_bound=2)
        )
    ...
    def test_if_si_greater_than_15(self):
        # If(SI == 0, 20, 10) > 15
        s1 = self.SI(bits=32, stride=1, lower_bound=0, upper_bound=2)
        ast_true = claripy.If(s1 == self.BVV(0, 32), self.BVV(20, 32), self.BVV(10, 32)) > self.BVV(15, 32)
        ast_false = claripy.If(s1 == self.BVV(0, 32), self.BVV(20, 32), self.BVV(10, 32)) <= self.BVV(15, 32)
    ...
    def test_if_si_greater_than_or_equal_15(self):
        # If(SI == 0, 20, 10) >= 15
        s1 = self.SI(bits=32, stride=1, lower_bound=0, upper_bound=2)
        ast_true = claripy.If(s1 == self.BVV(0, 32), self.BVV(15, 32), self.BVV(10, 32)) >= self.BVV(15, 32)
        ast_false = claripy.If(s1 == self.BVV(0, 32), self.BVV(15, 32), self.BVV(10, 32)) < self.BVV(15, 32)
    ...
    def test_extract_zero_extend_if_expr(self):
        ...
        s4 = self.SI(bits=64, stride=1, lower_bound=0, upper_bound=0xFFFFFFFFFFFFFFFF)
        ast_true = (
            claripy.Extract(
                0,
                0,
                claripy.ZeroExt(
                    32,
                    claripy.If(claripy.Extract(31, 0, (s4 & s4)).SLT(0), self.BVV(1, 32), self.BVV(0, 32)),
                ),
            )
            == 1
        )
        ...
        assert claripy.backends.vsa.identical(
            ...
            self.SI(bits=32, stride=1, lower_bound=-0x80000000, upper_bound=-1),
        )
        ...
        assert claripy.backends.vsa.identical(
            ...
            self.SI(bits=32, stride=1, lower_bound=0, upper_bound=0x7FFFFFFF),
        )

    def test_top_si_not_equal_neg1(self):
        # TOP_SI != -1
        s5 = self.SI(bits=32, stride=1, lower_bound=0, upper_bound=0xFFFFFFFF)
        ast_true = s5 == self.SI(bits=32, stride=1, lower_bound=0xFFFFFFFF, upper_bound=0xFFFFFFFF)
        ast_false = s5 != self.SI(bits=32, stride=1, lower_bound=0xFFFFFFFF, upper_bound=0xFFFFFFFF)
        ...
        assert claripy.backends.vsa.identical(
            trueside_replacement[0][1],
            self.SI(bits=32, stride=1, lower_bound=0xFFFFFFFF, upper_bound=0xFFFFFFFF),
        )
...
class TestVSADiscreteValueSet(unittest.TestCase):  # pylint: disable=no-member,function-redefined
    ...
    def test_intersection_operations(self):
        ...
        val_1 = self.SI(bits=32, stride=1, lower_bound=0, upper_bound=10)
        val_2 = self.SI(bits=32, stride=1, lower_bound=10, upper_bound=20)
        val_3 = self.SI(bits=32, stride=1, lower_bound=15, upper_bound=50)
        r = val_1.union(val_2)
        assert isinstance(vsa_model(r), DiscreteStridedIntervalSet)
    ...
    def test_logical_operations(self):
        # Logical operations
        val_1 = self.SI(bits=32, stride=1, lower_bound=0, upper_bound=10)
        val_2 = self.SI(bits=32, stride=1, lower_bound=5, upper_bound=20)
        r_1 = val_1.union(val_2)
        val_3 = self.SI(bits=32, stride=1, lower_bound=20, upper_bound=30)
        val_4 = self.SI(bits=32, stride=1, lower_bound=25, upper_bound=35)
    ...
    def test_arithmetic_operations(self):
        ...
        r = r_1 + r_2
        assert isinstance(vsa_model(r), DiscreteStridedIntervalSet)
        assert vsa_model(r).collapse().identical(vsa_model(self.SI(bits=32, stride=1, lower_bound=20, upper_bound=55)))
...
class TestSolution(unittest.TestCase):  # pylint: disable=no-member,function-redefined
    ...
    def test_solution_on_valueset(self):
        ...
        vs = self.VS(region="global", bits=32, val=0xDEADCA7)
    ...
    def test_solution_on_combined_valueset(self):
        ...
        vs = self.VS(bits=si.size(), region="foo", val=claripy.backends.vsa.convert(si))
        vs2 = self.VS(bits=si2.size(), region="foo", val=claripy.backends.vsa.convert(si2))
        vs = vs.union(vs2)
...
class TestVSAShiftingOperations(unittest.TestCase):  # pylint: disable=no-member,function-redefined
    ...
    def test_lshr_shift_by_1(self):
        ...
        si = self.SI(bits=32, stride=1, lower_bound=2, upper_bound=4)
        r = si.LShR(1)
        assert self.identical(r, self.SI(bits=32, stride=1, lower_bound=1, upper_bound=2))

    def test_lshr_shift_by_4(self):
        # <32>4[15,11] LShR 4 = <32>1[0, 0xfffffff]
        si = self.SI(bits=32, stride=4, lower_bound=15, upper_bound=11)
        r = si.LShR(4)
        assert self.identical(r, self.SI(bits=32, stride=1, lower_bound=0, upper_bound=0xFFFFFFF))

    def test_extract_operation(self):
        # Extract
        si = self.SI(bits=32, stride=4, lower_bound=15, upper_bound=11)
        r = si[31:4]
        assert self.identical(r, self.SI(bits=28, stride=1, lower_bound=0, upper_bound=0xFFFFFFF))

    def test_signed_shift_right(self):
        # <32>1[-4,-2] >> 1 = <32>1[-2,-1]
        si = self.SI(bits=32, stride=1, lower_bound=-4, upper_bound=-2)
        r = si >> 1
        assert self.identical(r, self.SI(bits=32, stride=1, lower_bound=-2, upper_bound=-1))
...
class TestReverseOperations(unittest.TestCase):  # pylint: disable=no-member,function-redefined
    ...
    def setUp(self):
        self.SI = claripy.SI
        self.StridedInterval = StridedInterval
        self.DiscreteStridedIntervalSet = DiscreteStridedIntervalSet
        self.SolverVSA = claripy.SolverVSA
        self.backend = claripy.backends.vsa

    def test_reverse_and_intersection(self):
        # Reverse and intersection operations on StridedIntervals
        x = self.SI(name="TOP", bits=64, lower_bound=0, upper_bound=0xFFFFFFFFFFFFFFFF, stride=1)  # TOP
        y = self.SI(name="range", bits=64, lower_bound=0, upper_bound=1337, stride=1)  # [0, 1337]
    ...
    def test_reverse_discrete_strided_interval_set(self):
        ...
        si0 = self.StridedInterval(name="a", bits=32, stride=0, lower_bound=0xFFFF0000, upper_bound=0xFFFF0000)
        si1 = self.StridedInterval(name="a", bits=32, stride=0, lower_bound=0xFFFF0001, upper_bound=0xFFFF0001)
        dsis = self.DiscreteStridedIntervalSet(name="b", bits=32, si_set={si0, si1})
