...
class Z3(Backend):
    ...
    def solver(self, timeout: int = 0, *, force_new: bool = False) -> Solver:
        ...
        '''
        if force_new:
            return Solver(claricpp.claricpp_backend_z3_new_tls_solver(self._raw, timeout))
        else:
            return Solver(claricpp.claricpp_backend_z3_tls_solver(self._raw, timeout))

    def add(self, solver: Solver, constraints: Union[Expr, List[Expr]], tracked: bool) -> None:
        '''
        ...
        if type(constraints) is Expr:
            ...
            if tracked:
                claricpp.claricpp_backend_z3_add_vec_tracked(self._raw, solver.raw, add, len(add))
            else:
                claricpp.claricpp_backend_z3_add_vec_untracked(self._raw, solver.raw, add, len(add))
        elif tracked:
            claricpp.claricpp_backend_z3_add_tracked(self._raw, solver.raw, constraint.raw)
        else:
            claricpp.claricpp_backend_z3_add_untracked(self._raw, solver.raw, constraint.raw)

    def satisfiable(self, solver: Solver, extra_constraints: Optional[List[Expr]] = None) -> bool:
        '''
        ...
        return bool(claricpp.claricpp_backend_z3_satisfiable_ec(self._raw, solver.raw, ec, len(ec)))

    def solution(self, expr: Expr, sol: Expr, solver: Solver, extra_constraitns: Optional[List[Expr]] = None) -> bool:
        '''
        Return true iff it is possible that sol == expr for solver given extra_constraints
        '''
        if extra_constraitns is None:
            return bool(claricpp.claricpp_backend_z3_solution(self._raw, expr.raw, sol.raw, solver.raw))
        ec = [ i.raw for i in extra_constraitns ]
        return bool(claricpp.claricpp_backend_z3_solution_ec(self._raw, expr.raw, sol.raw, solver.raw, ec, len(ec)))

    def min(self, expr: Expr, solver: Solver, signed: bool, extra_constraints: Optional[List[Expr]] = None) -> int:
        '''
        ...
        if extra_constraints is not None:
            ec = [ i.raw for i in extra_constraints ]
            if signed:
                return claricpp.claricpp_backend_z3_min_signed_ec(self._raw, expr.raw, solver.raw, ec, len(ec))
            return claricpp.claricpp_backend_z3_min_unsigned_ec(self._raw, expr.raw, solver.raw, ec, len(ec))
        elif signed:
            return claricpp.claricpp_backend_z3_min_signed(self._raw, expr.raw, solver.raw)
        return claricpp.claricpp_backend_z3_min_unsigned(self._raw, expr.raw, solver.raw)

    def max(self, expr: Expr, solver: Solver, signed: bool, extra_constraints: Optional[List[Expr]] = None) -> int:
        '''
        ...
        if extra_constraints is not None:
           ec = [ i.raw for i in extra_constraints ]
           if signed:
               return claricpp.claricpp_backend_z3_max_signed_ec(self._raw, expr.raw, solver.raw, ec, len(ec))
           return claricpp.claricpp_backend_z3_max_unsigned_ec(self._raw, expr.raw, solver.raw, ec, len(ec))
        elif signed:
            return claricpp.claricpp_backend_z3_max_signed(self._raw, expr.raw, solver.raw)
        return claricpp.claricpp_backend_z3_max_unsigned(self._raw, expr.raw, solver.raw)
    ...
    def eval(self, expr: Expr, solver: Solver, n_sol: int, extra_constraints: Optional[List[Expr]] = None) -> List[LazyPrim]:
        '''
        ...
        if extra_constraints is None:
            ...
            ec = [ i.raw for i in extra_constraints ]
            out = claricpp.claricpp_backend_z3_eval(self._raw, expr.raw, solver.raw, n, ec, len(ec))
        arr = out.arr
        # TODO: free array but not contents?
        return [ Prim(arr[i]) for i in range(arr.len) ]

    def batch_eval(self, exprs: List[Expr], solver: Solver, n_sol: int, extra_constraints: Optional[List[Expr]] = None) -> List[List[LazyPrim]]:
        '''
        ...
        if extra_constraints is None:
            out = claricpp.claricpp_backend_z3_batch_eval(self._raw, ex, len(ex), solver.raw, n_sol)
        else:
            ec = [ i.raw for i in extra_constraints ]
            out = claricpp.claricpp_backend_z3_batch_eval_ec(self._raw, ex, len(ex), solver.raw, n_sol, ec, len(ec))
