...
class Backend:
    ...
    @cached_property
    def name(self) -> str:
        return to_utf8(claricpp.claricpp_backend_name(self._backend))

    def handles(self, expr: Expr) -> bool:
        '''
        Returns true if and only if the particular backend can handle expr
        '''
        return bool(claricpp.claricpp_backend_handles(self._backend, expr.raw))

    def simplify(self, expr: Expr) -> Expr:
        '''
        Returns the Expr resulting from simplifying expr with self._backend
        '''
        return Expr(claricpp.claricpp_backend_simplify(self._backend, expr.raw))

    def downsize(self) -> None:
        '''
        Free memory by clearing caches and such within the backend
        '''
        claricpp.claricpp_backend_downsize(self._backend)
    ...
    def clear_persistent_data(self) -> None:
        ...
        '''
        claricpp.claricpp_backend_clear_persistent_data(self._backend)

    def get_big_int_mode(self) -> BigIntMode:
        '''
        Get the BigIntMode this backend is using
        '''
        return BigIntMode(claricpp.claricpp_backend_get_big_int_mode(self._backend))
    ...
    def set_big_int_mode(self, mode: BigIntMode) -> BigIntMode:
        ...
        '''
        return BigIntMode(claricpp.claricpp_backend_set_big_int_mode(self._backend, mode.value))
