...
void expr() {
    ...
    // Make like
    const auto ml_c { exc(claricpp_expr_make_like_annotations(e_c, API::to_c(ans()))) }; // NOLINT
    ...
    // bit length
    UNITTEST_ASSERT(exc(claricpp_expr_bit_length(e_c)) == e_len);

    // symbolic
    UNITTEST_ASSERT(exc(claricpp_expr_symbolic(e_c)));
    UNITTEST_ASSERT(!exc(claricpp_expr_symbolic(API::copy_to_c(Create::literal(true))))); // NOLINT

    // annotations
    UNITTEST_ASSERT(exc(claricpp_expr_annotations(e_c)).ptr == nullptr); // NOLINT (leak ok)
    UNITTEST_ASSERT(API::to_cpp_ref(exc(claricpp_expr_annotations(ml_c))).hash == ans()->hash);

    // args of zero extend
    const auto args { exc(claricpp_expr_args(API::to_c(Create::zero_ext(bv_sym, 3)))) }; // NOLINT
    ...
    // repr
    const std::string ml_c_repr { exc(claricpp_expr_repr(ml_c)) };
    const std::string ml_repr { ml->repr() };
    UNITTEST_ASSERT(ml_repr == ml_c_repr);

    // names
    UNITTEST_ASSERT(exc(claricpp_expr_type_name(ml_c)) == "BV"s);
    UNITTEST_ASSERT(exc(claricpp_expr_op_name(ml_c)) == "Sub"s);

    // CUIDs
    UNITTEST_ASSERT(exc(claricpp_expr_cuid(ml_c)) == ml->cuid);
    UNITTEST_ASSERT(exc(claricpp_expr_op_cuid(ml_c)) == ml->op->cuid);
}
