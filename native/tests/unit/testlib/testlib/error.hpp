...
namespace UnitTest::TestLib {
    ...
    extern thread_local std::shared_ptr<const Utils::Log::Backend::Base> original_bk;

    /** The original log style */
    extern thread_local std::shared_ptr<const Utils::Log::Style::Base> original_sty;
    ...
    template <typename... Args> void ut_fail(Args &&...args) {
        if (original_bk != nullptr) {
            ...
            Utils::Log::Backend::unsafe_set(std::move(original_bk));
        }
        if (original_sty != nullptr) {
            auto copy { original_sty }; // Just in case someone is dumb and catches the error
            Utils::Log::Style::unsafe_set(std::move(original_sty));
        }
        // Do not catch this
