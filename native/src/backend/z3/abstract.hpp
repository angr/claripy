...
namespace Backend::Z3 {
    ...
    /** Used to convert Z3 ASTs expressions into Claricpp expressions */
    template <typename Z3> struct Abstract final {
        ...
        static Expression::BasePtr internal(const z3::expr &b, const z3::func_decl &decl) {
            return Create::literal(internal_primitive(b, decl));
        }

        /** Abstraction function for Z3_OP_UNINTERPRETED */
        static Expression::BasePtr uninterpreted(const z3::expr &b_obj, const z3::func_decl &decl,
                    ...
                    case Z3_BV_SORT: {
                        ...
                        if (const auto lookup { satd.find(name_hash) }; lookup != satd.end()) {
                            return Create::symbol<Expression::BV>(
                                std::move(name), bl, Annotation::SPAV { lookup->second });
                        }
        ...
        template <bool B> static Expression::BasePtr bool_() {
            static Expression::BasePtr ret { nullptr };
            if (ret == nullptr) {
        ...
        static Expression::BasePtr compare(const ArgsVec &args) {
            ASSERT_ARG_LEN(args, 2);
        ...
        static Expression::BasePtr eq(const ArgsVec &args) {
            ASSERT_ARG_LEN(args, 2);
        ...
        static Expression::BasePtr distinct(const ArgsVec &args) {
            ASSERT_ARG_LEN(args, 2);
            switch (GET_EARG(0)->cuid) {
        ...
        static Expression::BasePtr ite(const ArgsVec &args) {
            ASSERT_ARG_LEN(args, 3);
        ...
        template <typename T> static Expression::BasePtr and_(ArgsVec &args) {
            FLAT_BINARY(Create::and_<T>);
        }

        /** Abstraction function for z3 or ops */
        template <typename T> static Expression::BasePtr or_(const ArgsVec &args) {
            FLAT_BINARY(Create::or_<T>);
        }

        /** Abstraction function for z3 xor ops */
        template <typename T> static Expression::BasePtr xor_(const ArgsVec &args) {
            FLAT_BINARY(Create::xor_);
        }

        /** Abstraction function for z3 not/inversion ops */
        template <typename T> static Expression::BasePtr not_(const ArgsVec &args) {
            if constexpr (std::is_same_v<T, Expression::Bool>) {
                UNARY(Create::not_);
            }
        ...
        template <typename T> static Expression::BasePtr neg(const ArgsVec &args) {
            UNARY(Create::neg<T>);
        }

        /** Abstraction function for Z3_OP_BADD */
        static Expression::BasePtr abs(const ArgsVec &args) { UNARY(Create::abs) }

        /** Abstraction function for Z3_OP_BADD */
        static Expression::BasePtr add(const ArgsVec &args) { FLAT_BINARY(Create::add); }

        /** Abstraction function for Z3_OP_BSUB */
        static Expression::BasePtr sub(const ArgsVec &args) { BINARY(Create::sub); }

        /** Abstraction function for Z3_OP_BMUL */
        static Expression::BasePtr mul(const ArgsVec &args) { FLAT_BINARY(Create::mul); }

        /** Abstraction function for z3 BV division */
        template <bool Signed> static Expression::BasePtr div(const ArgsVec &args) {
         ...
         */
        template <bool Signed> static Expression::BasePtr rem(const ArgsVec &args) {
        ...
        struct BV final {
            ...
            static Expression::BasePtr num(const z3::expr &b_obj, const Z3 &bk) {
...
#undef G_CASE
        ...
        template <Mode::Shift Mask> static Expression::BasePtr shift(const ArgsVec &args) {
            BINARY(Create::shift<Mask>);
        }

        /** Abstraction function for BV rotations */
        template <Mode::LR LR> static Expression::BasePtr rotate(const ArgsVec &args) {
        ...
        static Expression::BasePtr concat(const ArgsVec &args) {
            BINARY(Create::concat<Expression::BV>);
        }

        /** Abstraction function for Z3_OP_SIGN_EXT */
        static Expression::BasePtr sign_ext(const ArgsVec &args, const z3::func_decl &decl) {
            const auto &ctx { decl.ctx() };
            UINT_BINARY(Create::sign_ext);
        }

        /** Abstraction function for Z3_OP_ZERO_EXT */
        static Expression::BasePtr zero_ext(const ArgsVec &args, const z3::func_decl &decl) {
            const auto &ctx { decl.ctx() };
            UINT_BINARY(Create::zero_ext);
        }

        /** Abstraction function for Z3_OP_EXTRACT */
        static Expression::BasePtr extract(const ArgsVec &args, const z3::expr &e) {
            ASSERT_ARG_LEN(args, 1);
        ...
        struct FP final {
          private:
            ...
            static Expression::BasePtr fpa_literal(const z3::expr &b_obj, const double dbl,
                                                   const float flt) {
                const auto sort { b_obj.get_sort() };
                const auto width { z3_sort_to_fp_width(sort) };
                if (LIKELY(width == Mode::FP::dbl)) {
          ...
          public:
            ...
            static Expression::BasePtr to_bv(const ArgsVec &args, const z3::func_decl &decl) {
                ASSERT_ARG_LEN(args, 2);
            ...
            static Expression::BasePtr to_ieee_bv(const ArgsVec &args) {
             ...
             *  TODO
             */
            static Expression::BasePtr to_fp(const ArgsVec &args) {
                (void) args;
            ...
            static std::variant<double, float> num_primitive(const z3::expr &b_obj) {
                ...
                if (LIKELY(width == FP::dbl)) {
                    const uint64_t to_val {
                        (Utils::widen<uint64_t, true>(sign) << FP::dbl.no_sign_width()) |
                        mantissa | (Utils::unsign(exp) << FP::dbl.mantissa_raw())
                    };
            ...
            static Expression::BasePtr num(const z3::expr &b_obj) {
            ...
            template <Mode::Sign::FP Sign> static Expression::BasePtr zero(const z3::expr &b_obj) {
                return fpa_literal<Sign>(b_obj, 0., 0.F);
            }

            /** Abstraction function for fpa inf */
            template <Mode::Sign::FP Sign> static Expression::BasePtr inf(const z3::expr &b_obj) {
             ...
             */
            static Expression::BasePtr nan(const z3::expr &b_obj) {
            ...
            static Expression::BasePtr add(const ArgsVec &args) { MODE_BINARY(Create::FP::add); }

            /** Abstraction function for Z3_OP_FPA_SUB */
            static Expression::BasePtr sub(const ArgsVec &args) { MODE_BINARY(Create::FP::sub); }

            /** Abstraction function for Z3_OP_FPA_MUL */
            static Expression::BasePtr mul(const ArgsVec &args) { MODE_BINARY(Create::FP::mul); }

            /** Abstraction function for Z3_OP_FPA_DIV */
            static Expression::BasePtr div(const ArgsVec &args) { MODE_BINARY(Create::FP::div); }
        };
