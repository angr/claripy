...
def find_lib(base, name):
    ...
    files = [ i for i in files if (i.endswith('.so') or i.endswith('.dylib') or i.endswith('.dll')) ]
    if len(files) != 1:
...
def _build_native():
    ...
    cdefs_intermediate_f = info['CDEFS_SOURCE_F'] + '.i' # We care about the intermediate file
    ...
    if not is_make:
        print('Unknown if non-make generators support intermediate file targets', file=sys.stderr)
    run_cmd_no_fail(*makej, cdefs_target)
    # Include dirs for building ffi
    inc_guess = '/include/python' + str(sys.version_info.major) + '.' + str(sys.version_info.minor)
    include_dirs = inc_guess + ';' + info['Python_INCLUDE_DIRS']
    # Run build_ffi
    sys.path.append(d['native'])
    from build_ffi import build_ffi_wrapper
    sys.path.pop()
    build_ffi_wrapper(input_lib = claricpp, output_lib = claricpp_ffi, build_dir = d['build'], ffi_dir = d['ffi'], intermediate_f = cdefs_intermediate_f, include_dirs = include_dirs, verbose=True)
